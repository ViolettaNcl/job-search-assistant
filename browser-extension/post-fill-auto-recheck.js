...
function vjaSyncVisibleReadinessCounts(readiness) {
  if (!readiness) return;
  const review = Number(readiness.reviewCount || 0) + Number(readiness.failedCount || 0);
  ...
  try {
    ...
    if (filled !== null) {
      $("fillNote").textContent = failed
        ? `Filled and verified ${filled} safe field${filled === 1 ? "" : "s"}. ${failed} autofill attempt${failed === 1 ? "" : "s"} still need manual verification; ${review} other field${review === 1 ? "" : "s"} need review and ${blocked} are manual-only.`
        : `Filled and verified ${filled} safe field${filled === 1 ? "" : "s"}. ${review} field${review === 1 ? "" : "s"} need review and ${blocked} are manual-only. Existing answers were not overwritten.`;
    } else {
      const recoveredText = recovered
        ...
        : "";
      $("fillNote").textContent = review || failed || blocked
        ? `Checklist reconciled. ${review} need review, ${failed} autofill attempt${failed === 1 ? "" : "s"} need verification, and ${blocked} are manual-only.${recoveredText}`
        : `Checklist reconciled. No unresolved fields were detected by the assistant.${recoveredText} Review the full employer form and attachment before submitting.`;
    }
  } catch (error) {
