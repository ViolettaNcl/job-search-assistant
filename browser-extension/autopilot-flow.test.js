...
async function scenario({queued=false,pauseAfterImport=false,pending=false,letter=true,roles=['Junior C#','Junior C#']}={}) {
  ...
  const context={console,URL,Date,Promise,AbortController,setTimeout,clearTimeout,self:{},chrome:{runtime:{onInstalled:event,onStartup:event,onMessage:event,getURL:p=>'chrome-extension://fixture/'+p},alarms:{create:async()=>{},onAlarm:event},storage:{local,sync:{get:async()=>({apiBase:'http://localhost:8080',vjaHhBrowserSearch:true})}},tabs:{
    ...
    sendMessage:async(id,m)=>{if(m.type==='vjaSiteApplyReady')return {ok:true};const tab=tabs.get(id);if(m.type==='vjaReadHhDiscovery'){if(tab.url.includes('/search/'))return{pageUrl:tab.url,links:['https://hh.ru/vacancy/1','https://hh.ru/vacancy/2']};trace.push('read:'+tab.url);return{pageUrl:tab.url,vacancy:{title:roles[Number(new URL(tab.url).pathname.split('/').pop())-1],url:tab.url}};}
  ...
  await context.runBrowserAutopilot();return{trace,data,jobs:await context.applicationJobs()};
}
(async()=>{
  ...
  const unresolved=await scenario({pending:true});assert(!unresolved.trace.includes('receipt'));assert(!unresolved.trace.includes('read:https://hh.ru/vacancy/2'));assert(unresolved.jobs.some(j=>j.state==='waiting'),'unverified submission retains continuation, not a duplicate');
