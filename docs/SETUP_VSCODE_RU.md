...
Для HH после подключения OAuth и выбора резюме на главном дашборде появляется управляемый режим **«Запустить AI-ассистента»**. Пока backend запущен, он раз в 10 минут проверяет очередь и автоматически отправляет только подтверждённо подходящие вакансии выше выбранного порога, с дневным лимитом, защитой от дублей и отдельным сопроводительным письмом. Кнопка **«Остановить»** сразу ставит новые автоматические отклики на паузу.

Для международных источников откройте официальный application URL и нажмите в расширении **«Отправить отклик + письмо»**. Расширение анализирует вакансию, создаёт письмо, выбирает CV и нажимает финальную кнопку только при однозначно заполненной форме. CAPTCHA, неизвестный обязательный ответ или неясная кнопка останавливают сценарий для вашей проверки. Подтверждённый отклик записывается автоматически; при отсутствии надёжного подтверждения используйте **Record applied** только после визуальной проверки.
