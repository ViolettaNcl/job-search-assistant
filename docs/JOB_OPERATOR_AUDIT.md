...
## 2.7.9 — Windows dashboard alias fix and letter preference

Inspected main `bb6b090d0d47382ac512060d24ce992106633250`; branch `fix/dashboard-loopback-and-letter-style`. The Windows launcher opens `127.0.0.1:8080`, while extension defaults use `localhost:8080`. Strict origin string equality incorrectly denied the shipped configuration. The bridge now recognizes only explicit loopback aliases with identical protocol and port; unrelated hosts, ports, schemes, paths and subframes remain blocked. Added actual Chromium extension-to-backend coverage of the shipped alias combination, alongside worker acceptance/rejection regressions.

Removed code walkthrough/demo promises from RU/EN application writing and the strategy objection suggestion; removed generic conversation closings from the legacy letter path. AI instructions and output validation enforce the preference, falling back to the concise verified letter when an unwanted promise appears. GitHub/project links and the contact footer remain. Eligibility/mandatory requirements are still checked independently of dashboard origin access. No new candidate data or deployment.

## 2.7.10 — connection lifecycle and multiple-source visibility

Base main `2bec7f7fac2eafc40a55e07073287681a387bde7`, branch `fix/dashboard-bridge-recovery-and-source-tabs`. The timeout screenshot proves no acknowledgement reached the page; it does not identify which browser/profile was used. Audited failure paths: previously no pre-dispatch handshake, content scripts only attached at navigation, and Windows opened the default browser. Added a bounded handshake, version display, safe unknown-delivery state, idempotent bridge repair restricted to the configured loopback server/top frame, and Chrome preference in the launcher. The content bridge is independently registered at document_start. Chromium regression removes a live bridge, restores it without page reload, reinjects again and verifies only one backend preparation occurs. Dashboard regression proves absent extension means no dispatch, then reconnection permits normal action.

Moved the retained vacancy link next to Apply; added source tabs and visible source errors. Kept existing collectors. Removed Remotive's .NET-only prefilter in favor of existing technical career lanes; excluded senior/unrelated roles. One shared six-hour feed cache follows Remotive's advised maximum four requests/day during a running process (cache resets on application restart), with cooldown after errors. Optional Adzuna still requires keys. Actual external employer submission remains unpiloted; these tests use local fixtures, not candidate accounts.

References: https://developer.chrome.com/docs/extensions/develop/concepts/content-scripts ; https://github.com/remotive-com/remote-jobs-api ; https://support.google.com/chrome_webstore/answer/2664769 . No credentials or new personal values committed. No mobile deployment or LinkedIn added.

## 2.7.11 — dashboard stale task recovery

Inspected main aca14b725985e3533b3926e2e591a0085b4353ef; no open PRs. Preserved 2.7.10 bridge repair and source tabs. Found a global lock on any persisted plan, disabled-autopilot recovery skipped entirely, receipt persistence behind tab loading, and five repeated dispatches on a lost channel. Added per-vacancy private review retention, receipt-first recovery, stale cleanup before the enabled gate, manual priority at safe boundaries, and one dispatch followed by receipt polling. Unknown outcomes are never classified as successful or automatically resubmitted. Browser execution still needs the running local server and Chrome; no new platforms or deployment were added.

Validation: existing 28 Node suites plus new worker cases for expired/closed/orphan/popup state, closed-tab receipts, disabled-autopilot recovery, same-vacancy duplicate protection, manual priority and single dispatch. Added real Chromium stale-plan recovery and deterministic backend eligibility messaging tests. Live employer submission success is not established by these fixtures. Review entries intentionally require checking HH; no automatic retry/reset of an uncertain employer submission is exposed.

## 2.7.12 — concurrent application execution

Inspected main 10dafaa984845b46b5bb83a49df743dbce5fd059, no open PRs. The prior stale-state fix still used one shared plan and one worker mutex. Replaced only execution ownership with an indexed per-job persisted queue; retained qualification, writing, receipt criteria, HH form handling, discovery and backend history. Up to three browser jobs execute concurrently, with manual priority for queued requests and immediate queue draining. No profile/credentials were added to source.

Each content message is scoped to its top-level HH tab and plan id. Plan identity and letter are immutable to the page. Receipt persistence and cleanup affect only the owning job. Added pre-dispatch content readiness, managed navigation ownership, per-document promise coalescing, legacy migration, closed/reused-tab handling, per-vacancy deduplication and terminal state preservation across worker restarts. Queued jobs receive a fresh execution expiry on start. Pausing prevents queued automatic jobs from starting; explicit manual jobs continue.

Validation includes all prior 28 Node suites, a new concurrent worker scenario (one autopilot plus four manual jobs, three slots, staggered completion, a closed tab, distinct letters, exact receipts, duplicate clicks and reused tab ids), and real Chromium synthetic HH forms/worker restart in CI. Synthetic success is not a live HH reliability sample. Unknown sends stay under manual review; arbitrary unlimited concurrency and CAPTCHA bypass are not implemented.
